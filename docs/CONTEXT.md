...
| 书单墙篇数全 0 | `listAsTree()` 子节点 `postCount` 不可信 | ⚠️ **当前仓库仍写 `book.postCount`**，应收口为 `getByName(...).postCount`（见下节待办） |
...
1. **修书单墙篇数**：`index.html` 书单项改为对每个 `book` 使用 `categoryFinder.getByName(book.metadata.name).postCount`（或等价写法），避免再显示全 0。  
2. **主题版本号**：`theme.yaml` 仍为 `1.0.0`，样式已含 v1.1 字号/栅格修复，建议 bump 并同步 `?v=` 缓存。  
3. **视觉精修**（用户意向）：在保留书卷气前提下优化间距、动效、阅读体验；勿推翻 token / 信息架构除非明确改方向。  
4. **移动端**：880px 断点已有；可再验书单墙双列、文章侧栏顺序、播放器触控。  
5. **可选**：标签页、搜索页、友情链接等若站点启用插件再补模板。  
6. **运维**：CORS 若从 `*` 收紧，只改 Navidrome 环境变量。
